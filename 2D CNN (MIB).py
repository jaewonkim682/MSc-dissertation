...
while 1:
    ...
    train_infected, test_infected, train_infected_result, test_infected_result = train_test_split(
     infected_multispectral, result_infected, test_size=0.1,random_state=10, shuffle=True)
    
    train_uninfected, test_uninfected, train_uninfected_result, test_uninfected_result = train_test_split(
     uninfected_multispectral, result_uninfected, test_size=0.1,random_state=10, shuffle=True)
    ...
    test_shuffle, test_result_shuffle=shuffle(test,test_result)
    
    #build the model
    model = Sequential()
    model.add(Conv2D(16, (4,4), 1, activation = 'relu', input_shape=(24,24,1)))
    ...
    hist = model.fit(train_shuffle, train_result_shuffle, validation_split=0.2, epochs = 90, batch_size=32) 
    ...
    test_loss, test_acc = model.evaluate(test_shuffle, test_result_shuffle, batch_size=8)
