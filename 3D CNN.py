...
while 1:
    ...
    #reshape multispectral images into (24,24,1)     
    temp=0
    infected_multispectral=[]
    for i in range(no_multispectral_image_infected):
        temp=infected_images[i]
        temp=temp[:,:,newaxis]
        infected_multispectral.append(temp)
    
    uninfected_multispectral=[]
    temp=0
    for i in range(no_multispectral_image_uninfected):
        temp=uninfected_images[i]
        temp=temp[:,:,newaxis]
        uninfected_multispectral.append(temp)
        
    #combine multispectral images in each patch
    infected_patch=np.array_split(infected_multispectral,no_multispectral_image_infected/14)
    uninfected_patch=np.array_split(uninfected_multispectral,no_multispectral_image_uninfected/14)
    ...
    train_suffle, train_result_suffle=shuffle(train,train_result)
    test_shffule, test_result_suffle=shuffle(test,test_result)
    
    #build the model
    model = Sequential()
    model.add(Conv3D(16, (3,3,3), 1, activation = 'relu', input_shape=(14,24,24,1)))
    model.add(Conv3D(16, (3,3,3), 1, activation = 'relu'))
    ...
    hist = model.fit(train_suffle, train_result_suffle, validation_split=0.2, epochs = 30, batch_size=32)
    ...
    test_loss, test_acc = model.evaluate(test_shffule, test_result_suffle, batch_size=8)
