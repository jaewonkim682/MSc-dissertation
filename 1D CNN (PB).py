...
while 1:
    ...
    #concatenate arrays in each patch 
    temp=0
    infected_val_combine=[]
    i=0
    for j in range(infected_patch_total_number):
        i=j*14
        temp=np.concatenate([infected_flatten_multispectral[i],infected_flatten_multispectral[i+1],infected_flatten_multispectral[i+2],
                             infected_flatten_multispectral[i+3],infected_flatten_multispectral[i+4],infected_flatten_multispectral[i+5],
                             infected_flatten_multispectral[i+6],infected_flatten_multispectral[i+7],infected_flatten_multispectral[i+8],
                             infected_flatten_multispectral[i+9],infected_flatten_multispectral[i+10],infected_flatten_multispectral[i+11],
                             infected_flatten_multispectral[i+12],infected_flatten_multispectral[i+13]])
        infected_val_combine.append(temp)
    
    temp=0
    uninfected_val_combine=[]
    i=0
    for j in range(uninfected_patch_total_number):
        i=j*14
        temp=np.concatenate([uninfected_flatten_multispectral[i],uninfected_flatten_multispectral[i+1],uninfected_flatten_multispectral[i+2],
                             uninfected_flatten_multispectral[i+3],uninfected_flatten_multispectral[i+4],uninfected_flatten_multispectral[i+5],
                             uninfected_flatten_multispectral[i+6],uninfected_flatten_multispectral[i+7],uninfected_flatten_multispectral[i+8],
                             uninfected_flatten_multispectral[i+9],uninfected_flatten_multispectral[i+10],uninfected_flatten_multispectral[i+11],
                             uninfected_flatten_multispectral[i+12],uninfected_flatten_multispectral[i+13]])
        uninfected_val_combine.append(temp)     
    
    #produce result tables
    result_infected=[]
    for i in range(infected_patch_total_number):
        result_infected.append(0)
    result_uninfected=[]
    for i in range(uninfected_patch_total_number):
    ...
    train_infected, test_infected, train_infected_result, test_infected_result = train_test_split(
     infected_val_combine, result_infected, test_size=0.1,random_state=10, shuffle=True)
    
    train_uninfected, test_uninfected, train_uninfected_result, test_uninfected_result = train_test_split(
     uninfected_val_combine, result_uninfected, test_size=0.1,random_state=10, shuffle=True)
    ...
    #build the model
    model = Sequential()
    model.add(Conv1D(16, 5, activation = 'relu', input_shape=[8064,1]))
    ...
    hist = model.fit(train_shuffle, train_result_shuffle, validation_split=0.2, epochs = 50, batch_size=8)
